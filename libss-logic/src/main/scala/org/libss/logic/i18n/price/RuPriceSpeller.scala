package org.libss.logic.i18n.price

import java.text.{DecimalFormat, DecimalFormatSymbols}

/**
  * Created by Kaa 
  * on 07.06.2016 at 02:06.
  */
trait StringHelper {
  ...
  implicit def toNullOrEmpty(str: String) = new {
    def isNullOrEmpty = !Option(str).exists(_.nonEmpty)
  }
}

object RuNumberSpeller extends StringHelper with CentsExtractor {
  ...
  protected def spellIntegerPart(number: BigDecimal) = capitalize(spellNumber(number.toBigInt()))
  ...
  def spellNumber(number: scala.BigInt, zeroOrder: OrderName = OrderName("рубль", "рубля", "рублей", true)) = {
    if (number == 0) "ноль " + zeroOrder.spellingFrom5 else
  ...
  def capitalize(s: String) = s.headOption.map(_.toUpper + s.tail).getOrElse("")
...
trait PluralForms {
  ...
  def pluralEnding(base: String, number:Long) = base ++ (number match {
    case n if n % 10 == 1 && n%100 !=11 => ""
    case n if (n % 10 >= 2) && (n % 10 <= 4) && (n % 100 < 10 || n % 100 >= 20 ) => "а"
    case n => "ов"
  })
}
...
object RuPriceSpeller extends StringHelper with CentsExtractor {
  val formatSymb = new DecimalFormatSymbols()
  formatSymb.setGroupingSeparator(' ')
  val formatter = new DecimalFormat("###,###", formatSymb)

  def spellShortPrice(price: BigDecimal) = {
    val c = extractCents(price)
    if (c != "00") formatter.format(price.doubleValue().floor) + " Р. " + c + " к."
    else formatter.format(price) + " Р."
  }

  def spellPrice(price: BigDecimal) = {
    RuNumberSpeller.capitalize(
      RuNumberSpeller.spellNumber(price.toBigInt())
    ) +?+ {
      val c = extractCents(price)
      c + " к."
    }
